...
from components.heatwhisper import sensor as hw_sensor  # noqa: E402
from components.heatwhisper import number as hw_number  # noqa: E402
...
def test_sensor_to_code_wiring():
    assert asyncio.iscoroutinefunction(hw_sensor.to_code)
    parent, var = MagicMock(name="parent"), MagicMock(name="var")
    hw_sensor.cg.get_variable = AsyncMock(return_value=parent)
    hw_sensor.cg.register_component = AsyncMock()
    _sensor.new_sensor.return_value = var
    _run(hw_sensor.to_code({"heatwhisper_id": "heatwhisper_bridge", "register": 40004}))
...
def test_number_to_code_wiring():
    assert asyncio.iscoroutinefunction(hw_number.to_code)
    parent, var = MagicMock(name="parent"), MagicMock(name="var")
    hw_number.cg.get_variable = AsyncMock(return_value=parent)
    hw_number.cg.register_component = AsyncMock()
    _number.new_number.return_value = var
    cfg = {"heatwhisper_id": "heatwhisper_bridge", "register": 43005,
           "min_value": -3000.0, "max_value": 3000.0, "step": 10.0}
    _run(hw_number.to_code(cfg))
...
def test_sensor_schema_rejects_unknown_register_warning():
    import os
    src = open(os.path.join(os.path.dirname(__file__), "..", "components", "heatwhisper", "sensor.py")).read()
    assert "is_known" in src

def test_number_schema_rejects_unknown_register_warning():
    import os
    src = open(os.path.join(os.path.dirname(__file__), "..", "components", "heatwhisper", "number.py")).read()
    assert "is_known" in src
