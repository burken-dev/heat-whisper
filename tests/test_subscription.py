...
CPP = open(os.path.join(REPO2, "components", "heatpump", "heatpump.cpp")).read()
...
def test_number_control_still_queues_when_disabled():
    ...
    body = CPP.split("HeatpumpNumber::control")[1].split("}  // namespace")[0]
...
def _base_names_table():
    # HP_FACTORY_NAMES is the post-base.yaml name authority; extract addr->name.
    import re
    cpp = open(os.path.join(os.path.dirname(__file__), "..", "components", "heatpump", "heatpump.cpp")).read()
    block = cpp.split("HP_FACTORY_NAMES[] = {", 1)[1].split("};", 1)[0]
...
def test_base_yaml_has_no_static_entities():
    ...
    base = _base_text()
    assert "platform: heatpump" not in base
...
def test_factory_covers_old_entity_set():
    ...
    from components.heatpump.registers import DEFAULT_ENABLED
