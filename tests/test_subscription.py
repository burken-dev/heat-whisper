...
CPP = open(os.path.join(REPO2, "components", "heatwhisper", "heatwhisper.cpp")).read()
...
def _base_names_table():
    # HW_FACTORY_NAMES is the post-base.yaml name authority; extract addr->name.
    import re
    cpp = open(os.path.join(os.path.dirname(__file__), "..", "components", "heatwhisper", "heatwhisper.cpp")).read()
    block = cpp.split("HW_FACTORY_NAMES[] = {", 1)[1].split("};", 1)[0]
...
def test_base_yaml_has_no_static_entities():
    ...
    base = _base_text()
    assert "platform: heatwhisper" not in base
...
def test_factory_covers_old_entity_set():
    ...
    from components.heatwhisper.registers import DEFAULT_ENABLED
